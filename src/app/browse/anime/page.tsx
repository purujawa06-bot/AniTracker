...
async function AnimeGrid({ genreId, page }: { genreId: string; page: number }) {
  const { data: mediaList, pagination } = await searchMedia('', { type: 'anime', genre: genreId, page, limit: 24 });
  const genres = await getGenres('anime');
  ...
  return (
    <div>
        <h1 className="text-3xl font-bold mb-6">
            Browsing Anime Genre: <span className="text-primary">{genre?.name}</span>
        </h1>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {mediaList.map((media) => (
        ...
        <div className="mt-8 flex justify-center">
            <PaginationComponent 
                ...
                buildLink={(p) => `/browse/anime?genre=${genreId}&page=${p}`}
            />
        </div>
    </div>
  );
  ...
  return (
    <div className="container py-8">
      {genreId ? (
        ...
        <>
            <h1 className="text-3xl font-bold mb-6">Browse Anime by Genre</h1>
            <GenreList />
        </>
      )}
    </div>
