    ...
    return (
      <div className="container py-24 text-center">
        ...
        <p className="text-muted-foreground mt-2 mb-6">
          Start by searching for an anime and adding it to your list.
        </p>
        <Link href="/">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {watchlist.map((item) => (
          <Card key={item.mal_id} className="flex flex-col">
             <Link href={`/anime/${item.mal_id}`} className="group block">
            ...
            <CardHeader>
              <CardTitle className="line-clamp-2 h-12">
                <Link href={`/anime/${item.mal_id}`} className="hover:text-primary transition-colors">
                    {item.title}
                </Link>
              </CardTitle>
            </CardHeader>
            <CardContent className="flex-grow">
              <div className="text-sm text-muted-foreground mb-2">
                {item.watchedEpisodes} / {item.episodes || '??'} watched
              </div>
              <Progress value={item.episodes > 0 ? (item.watchedEpisodes / item.episodes) * 100 : 0} />
            </CardContent>
            <CardFooter>
